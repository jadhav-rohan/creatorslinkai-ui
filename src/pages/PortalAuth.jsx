...
export default function PortalAuth({persona,mode}){
  ...
  async function submit(event){event.preventDefault();if(loading||!validate())return;setLoading(true);setRequestId(null);try{await authenticatePortal(persona,mode,form);const intended=location.state?.from,validIntended=intended&&(brand?intended.startsWith("/brand/"):intended.startsWith("/creator/"));navigate(validIntended?intended:(brand?"/brand/discovery":"/creator/dashboard"),{replace:true})}catch(error){setRequestId(error.requestId);const message=error.status===401?`Invalid credentials or this account does not have access to the ${brand?"Brand":"Creator"} portal.`:error.status===409?"This email is already registered. Sign in instead.":error.status===429?"Too many attempts. Please wait and try again.":error.status>=500?"The service is temporarily unavailable. Please try again.":error.message;setErrors(current=>({...current,form:message}))}finally{setLoading(false)}}
