...
function CreatorCard({creator,busy,onUpdate,onRemove,onWorkflowChange,workflowSaving,onDragStart,deliverableProps,canEdit}){
 ...
 return <article draggable={canEdit&&!workflowSaving} onDragStart={event=>{if(event.target.closest("button,input,select,textarea,a")){event.preventDefault();return}onDragStart(event,creator)}} className={`border-2 border-zinc-900 bg-white p-4 shadow-[3px_3px_0_#18181b] ${canEdit&&!workflowSaving?"cursor-grab active:cursor-grabbing":""} ${workflowSaving?"opacity-60":""}`}>
...
</div>}{error&&<p className="mt-3 text-xs text-red-400">{error}</p>}{canEdit&&<label className="mt-4 block text-xs font-bold text-zinc-600">Move to<select value={workflowOf(creator)} disabled={workflowSaving} onChange={event=>onWorkflowChange(creator,event.target.value)} className="brutal-field mt-1 w-full text-sm"><option value="ACTIVE">Active</option><option value="SCRIPTING">Scripting</option><option value="VIDEO_REVIEW">Video Review</option><option value="LIVE">Live</option><option value="PAYMENT">Payment</option></select></label>}<div className="mt-5 flex gap-2">{canEdit&&(editing?<>
...
export default function CampaignDetails(){
 ...
 const {campaignId}=useParams();const {token,logout}=useAuth();const {selectedWorkspace,selectedWorkspaceId,reloadWorkspaces}=useWorkspace();const navigate=useNavigate();const location=useLocation();const [campaign,setCampaign]=useState(null);const [loading,setLoading]=useState(true);const [error,setError]=useState(null);const [notice,setNotice]=useState(null);const [notFound,setNotFound]=useState(false);const [busy,setBusy]=useState(false);const [workflowSaving,setWorkflowSaving]=useState(new Set());const [editing,setEditing]=useState(false);const [lists,setLists]=useState([]);const [importList,setImportList]=useState("");const [importOpen,setImportOpen]=useState(false);const [catalogQuery,setCatalogQuery]=useState("");const [catalog,setCatalog]=useState([]);const [pickerOpen,setPickerOpen]=useState(false);
 ...
 async function updateCreator(id,payload){if(!canEditCampaign)return;setBusy(true);try{const updated=await campaignService.updateCreator(selectedWorkspaceId,campaignId,id,payload,token);setCampaign(c=>({...c,creators:c.creators.map(x=>x.creatorProfileId===id?{...x,...updated,workflowStatus:workflowOf({...x,...updated})}:x)}))}finally{setBusy(false)}}
 async function moveCreatorWorkflow(creator,target){if(!canEditCampaign||workflowSaving.has(creator.creatorProfileId)||workflowOf(creator)===target)return;const id=creator.creatorProfileId,previous=workflowOf(creator);setError(null);setWorkflowSaving(current=>new Set(current).add(id));setCampaign(current=>({...current,creators:current.creators.map(item=>item.creatorProfileId===id?{...item,workflowStatus:target}:item)}));try{const updated=await campaignService.updateCreator(selectedWorkspaceId,campaignId,id,{workflowStatus:target},token);setCampaign(current=>({...current,creators:current.creators.map(item=>item.creatorProfileId===id?{...item,...updated,workflowStatus:updated?.workflowStatus||target}:item)}));setNotice(`@${creator.instagramUsername} moved to ${workflowLabel(target)}.`)}catch(err){setCampaign(current=>({...current,creators:current.creators.map(item=>item.creatorProfileId===id?{...item,workflowStatus:previous}:item)}));if(err.status===401)logout();else setError(err.status>=500?"The creator could not be moved. Their previous workflow stage was restored.":`${err.message} The previous workflow stage was restored.`)}finally{setWorkflowSaving(current=>{const next=new Set(current);next.delete(id);return next})}}
...
function workflowOf(creator){return KANBAN_COLUMNS.some(column=>column.key===creator?.workflowStatus)?creator.workflowStatus:"ACTIVE"}function workflowLabel(value){return KANBAN_COLUMNS.find(column=>column.key===value)?.label||"Active"}
function CampaignKanban({creators,busy,workflowSaving,onWorkflowChange,onUpdate,onRemove,canEdit,makeDeliverableProps}){const [dragged,setDragged]=useState(null);return <section aria-labelledby="campaign-board-title"><p className="brutal-overline">Campaign board</p><h2 id="campaign-board-title" className="mt-2 text-2xl font-black">Creator workflow</h2><p className="mt-2 text-sm text-zinc-600">Drag creators between workflow stages or use each card’s Move to selector. Commercial participation status remains independent.</p><div className="mt-6 overflow-x-auto pb-4"><div className="grid min-w-[1320px] grid-cols-5 gap-4">{KANBAN_COLUMNS.map(column=>{const items=creators.filter(creator=>workflowOf(creator)===column.key);return <section key={column.key} onDragOver={event=>{if(canEdit)event.preventDefault()}} onDrop={event=>{event.preventDefault();const creator=creators.find(item=>item.creatorProfileId===dragged);if(creator)onWorkflowChange(creator,column.key);setDragged(null)}} aria-labelledby={`column-${column.key}`} className="min-h-[420px] border-2 border-zinc-900 bg-white"><header className={`flex min-h-14 items-center justify-between gap-3 border-b-2 border-zinc-900 px-4 ${column.color}`}><h3 id={`column-${column.key}`} className="text-sm font-black uppercase tracking-wide">{column.label}</h3><span className="flex h-7 min-w-7 items-center justify-center rounded-full bg-zinc-900 px-2 font-mono text-xs font-black text-white">{items.length}</span></header><div className="min-h-[350px] space-y-4 p-3">{items.map(creator=><CreatorCard key={creator.creatorProfileId} creator={creator} busy={busy} workflowSaving={workflowSaving.has(creator.creatorProfileId)} onDragStart={(event,item)=>{event.dataTransfer.effectAllowed="move";event.dataTransfer.setData("text/plain",item.creatorProfileId);setDragged(item.creatorProfileId)}} onWorkflowChange={onWorkflowChange} onUpdate={onUpdate} onRemove={onRemove} canEdit={canEdit} deliverableProps={makeDeliverableProps(creator)}/>)}{!items.length&&<p className="p-4 text-center text-xs text-zinc-400">Drop a creator here or use the Move to selector.</p>}</div></section>})}</div></div></section>}
