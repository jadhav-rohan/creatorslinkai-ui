...
function MediaPicker({igUserId,token,value,onChange,logout}){
  ...
  if(loading)return <div className="sm:col-span-2"><p className="font-bold">Select a Reel or post *</p><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{[1,2,3,4].map(item=><div key={item} className="aspect-[4/5] animate-pulse border-2 border-zinc-900 bg-zinc-200"/>)}</div></div>;
  ...
  return <div className="sm:col-span-2"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="font-bold">Select a Reel or post *</p><p className="mt-1 text-sm text-zinc-600">Choose one connected-account post for this keyword rule.</p></div><button type="button" onClick={refresh} className="border-2 border-zinc-900 bg-white px-4 py-2 text-sm font-black">Refresh media</button></div><div className="mt-4 flex gap-2" role="group" aria-label="Filter Instagram media">{[["ALL","All"],["REELS","Reels"],["POSTS","Posts"]].map(([key,label])=><button key={key} type="button" aria-pressed={filter===key} onClick={()=>setFilter(key)} className={`border-2 border-zinc-900 px-4 py-2 text-sm font-black ${filter===key?"bg-yellow-300 shadow-[3px_3px_0_#18181b]":"bg-white"}`}>{label}</button>)}</div>{!items.length?<div className="mt-4 border-2 border-dashed border-zinc-400 p-8 text-center"><p className="font-black">No eligible Instagram media found. Publish a Reel or feed post, then refresh this page.</p><button type="button" onClick={refresh} className="brutal-button mt-5">Refresh media</button></div>:!visible.length?<div className="mt-4 border-2 border-dashed border-zinc-400 p-7 text-center text-zinc-600">No media matches this filter.</div>:<div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{visible.map(item=>{const selected=value===item.mediaId,caption=item.caption?.trim()||"No caption";return <article key={item.mediaId} className={`relative overflow-hidden border-2 border-zinc-900 bg-white ${selected?"ring-4 ring-yellow-300 shadow-[5px_5px_0_#18181b]":""}`}><button type="button" onClick={()=>onChange(item.mediaId)} aria-pressed={selected} className="block w-full text-left"><div className="aspect-square border-b-2 border-zinc-900"><MediaPreview item={item} failed={failed.has(item.mediaId)} onFail={()=>setFailed(current=>new Set(current).add(item.mediaId))}/></div><div className="p-3"><div className="flex items-center justify-between gap-2"><span className="border border-zinc-900 bg-sky-100 px-2 py-1 text-[10px] font-black">{contentLabels[item.contentType]||item.contentType||"Media"}</span>{selected&&<span className="flex items-center gap-1 bg-yellow-300 px-2 py-1 text-[10px] font-black"><Check size={12}/> Selected</span>}</div><p className="mt-3 line-clamp-2 min-h-10 text-sm font-bold">{caption}</p><p className="mt-2 text-xs text-zinc-500">{item.publishedAt?new Date(item.publishedAt).toLocaleDateString():"Publication date unavailable"}</p></div></button>{item.permalink&&<a href={item.permalink} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()} aria-label={`View ${contentLabels[item.contentType]||"media"} on Instagram`} className="absolute right-2 top-2 flex h-9 w-9 items-center justify-center border-2 border-zinc-900 bg-white"><ExternalLink size={16}/></a>}</article>})}</div>}</div>;
}
