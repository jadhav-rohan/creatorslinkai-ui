...
<label className="block font-bold">{form.responseType==="GENERIC_TEMPLATE"?"Initial private reply":"Private DM message"} *<textarea value={form.dmMessage} onChange={e=>setForm(x=>({...x,dmMessage:e.target.value}))} required rows={5} placeholder={form.responseType==="GENERIC_TEMPLATE"?"Thanks! Reply to this message to see the collection.":undefined} className="brutal-field mt-2 w-full"/>{form.responseType==="GENERIC_TEMPLATE"&&<span className="mt-2 block text-xs font-normal text-zinc-600">Sent immediately after the keyword comment. The recipient must reply before Instagram permits the carousel to be sent.</span>}</label>
