...
export default function VerifyEmail(){
 ...
 return <main className="brutal-page flex min-h-screen items-center justify-center p-6"><section className="brutal-card w-full max-w-xl p-7 text-center md:p-10">
  ...
  {state==="success"&&<><h1 className="mt-4 text-4xl font-black">Email verified successfully.</h1><p className="mt-4 text-zinc-600">Your account is ready. Sign in through the portal you registered for.</p>{loginPath?<Link to={loginPath} className="brutal-button mt-7 inline-flex">Continue to sign in</Link>:<div className="mt-7 grid gap-3 sm:grid-cols-2"><Link to="/creator/login" className="brutal-button justify-center">Creator sign in</Link>{featureFlags.brandPortalEnabled?<Link to="/brand/login" className="border-2 border-zinc-900 bg-white p-3 font-black">Brand / Agency sign in</Link>:<Link to="/brand/coming-soon" className="border-2 border-zinc-400 bg-zinc-100 p-3 font-black text-zinc-600">Brand / Agency · Coming Soon</Link>}</div>}</>}
  {state==="brand-disabled"&&<><span className="mt-5 inline-flex border-2 border-zinc-900 bg-sky-200 px-3 py-1 text-xs font-black uppercase">Coming Soon</span><h1 className="mt-5 text-4xl font-black">Your email has been verified.</h1><p className="mt-4 text-zinc-600">{error||"The Brand and Agency portal is coming soon."}</p><Link to="/login" className="brutal-button mt-7 inline-flex">Back to public home</Link></>}
