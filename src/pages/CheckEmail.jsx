...
export default function CheckEmail(){
 ...
 if(persona==="BRAND"&&!featureFlags.brandPortalEnabled)return <main className="brutal-page flex min-h-screen items-center justify-center p-6"><section className="brutal-card w-full max-w-xl p-7 text-center md:p-10"><span className="inline-flex border-2 border-zinc-900 bg-sky-200 px-3 py-1 text-xs font-black uppercase">Coming Soon</span><h1 className="mt-5 text-4xl font-black">Brand and Agency access is not open yet.</h1><p className="mt-4 text-zinc-600">We are launching the Creator experience first. Brand verification resend is unavailable during MVP-1.</p><Link to="/login" className="brutal-button mt-7 inline-flex">Back to public home</Link></section></main>;
 return <main className="brutal-page flex min-h-screen items-center justify-center p-6"><section className="brutal-card w-full max-w-xl p-7 md:p-10">
  ...
  {persona?<Link to={loginPath} className="mt-5 block text-center text-sm font-black underline">Back to {persona==="BRAND"?"Brand / Agency":"Creator"} sign in</Link>:<div className="mt-5 grid gap-3 sm:grid-cols-2"><Link to="/creator/login" className="border-2 border-zinc-900 bg-white p-3 text-center text-sm font-black">Creator sign in</Link>{featureFlags.brandPortalEnabled?<Link to="/brand/login" className="border-2 border-zinc-900 bg-white p-3 text-center text-sm font-black">Brand / Agency sign in</Link>:<Link to="/brand/coming-soon" className="border-2 border-zinc-400 bg-zinc-100 p-3 text-center text-sm font-black text-zinc-600">Brand / Agency · Coming Soon</Link>}</div>}
 </section></main>;
}
